//! The 2D [`PhysicsWorld`]: owns all Rapier state and drives the fixed step.
...
use super::convert::{
    ...
    na_to_vec, point_to_vec, rb_to_handle, transform_to_iso, vec_to_na,
};
...
impl PhysicsWorld {
    ...
    fn gravity_na(&self) -> Vector2<f32> {
    ...
    pub fn add_rigid_body(&mut self, desc: RigidBodyDesc) -> RigidBodyHandle {
        let mut builder = rp::RigidBodyBuilder::new(Self::rapier_body_type(desc.body_type))
            ...
            .angvel(desc.velocity.angular)
            .linear_damping(desc.linear_damping)
            .angular_damping(desc.angular_damping)
            .ccd_enabled(desc.ccd_enabled)
            .can_sleep(desc.can_sleep);
        builder =
            builder.enabled_translations(!desc.locks.translation.x, !desc.locks.translation.y);
        if desc.locks.rotation {
    ...
    fn build_collider(desc: &ColliderDesc) -> PhysicsResult<rp::Collider> {
        let mut builder = match &desc.shape {
            ...
            Shape::Cuboid { half_extents } => {
                rp::ColliderBuilder::cuboid(half_extents.x, half_extents.y)
            }
            Shape::Capsule {
                half_height,
                radius,
            ...
            Shape::ConvexHull { points } => {
                // parry panics (rather than returning None) on fewer than 3 points.
                if points.len() < 3 {
                    return Err(PhysicsError::InvalidShape {
                        context: "convex hull needs at least 3 points",
                    });
                }
                let pts: Vec<na::Point2<f32>> =
    ...
    pub fn add_fixed_joint(
        ...
        let data = rp::FixedJointBuilder::new()
            .local_frame1(na::Isometry2::from_parts(
                na::Translation2::from(vec_to_na(j.local_anchor_a)),
                na::UnitComplex::new(j.local_frame_a),
            ))
            .local_frame2(na::Isometry2::from_parts(
                na::Translation2::from(vec_to_na(j.local_anchor_b)),
                na::UnitComplex::new(j.local_frame_b),
            ));
        let handle =
            self.impulse_joints
    ...
    pub fn add_revolute_joint(
        ...
        let mut builder = rp::RevoluteJointBuilder::new()
            .local_anchor1(na::Point2::from(vec_to_na(j.local_anchor_a)))
            .local_anchor2(na::Point2::from(vec_to_na(j.local_anchor_b)));
    ...
    pub fn body_transform(&self, h: RigidBodyHandle) -> Option<Transform2D> {
        self.bodies
            .get(handle_to_rb(h))
            .map(|b| iso_to_transform(b.position()))
    }

    pub fn set_body_transform(&mut self, h: RigidBodyHandle, t: Transform2D) -> bool {
    ...
    pub fn set_body_velocity(&mut self, h: RigidBodyHandle, v: Velocity) -> bool {
        match self.bodies.get_mut(handle_to_rb(h)) {
            Some(body) => {
                ...
                body.set_angvel(v.angular, true);
                true
            }
            None => false,
        }
    ...
    pub fn apply_force(&mut self, h: RigidBodyHandle, force: Vec2) -> bool {
        match self.bodies.get_mut(handle_to_rb(h)) {
            Some(body) => {
    ...
    pub fn apply_impulse(&mut self, h: RigidBodyHandle, impulse: Vec2) -> bool {
        match self.bodies.get_mut(handle_to_rb(h)) {
            Some(body) => {
    ...
    pub fn apply_torque(&mut self, h: RigidBodyHandle, torque: f32) -> bool {
        match self.bodies.get_mut(handle_to_rb(h)) {
            Some(body) => {
    ...
    pub fn ray_cast(&self, ray: Ray, max_toi: f32, filter: QueryFilter) -> Option<RayHit> {
        ...
        let rapier_ray = rp::Ray::new(na::Point2::from(vec_to_na(ray.origin)), vec_to_na(dir));
    ...
    fn build_query_shape(shape: &Shape) -> PhysicsResult<rp::SharedShape> {
        match shape {
            ...
            Shape::Cuboid { half_extents } => {
                Ok(rp::SharedShape::cuboid(half_extents.x, half_extents.y))
            }
            Shape::Capsule {
                half_height,
                radius,
