...
The asset subsystem turns logical file paths into typed, reference-counted, hot-reloadable runtime objects without ever blocking the main thread. It provides stable content-independent identity for assets, an `AssetServer` that dispatches file reads to a background IO thread pool, an `AssetLoader` trait for converting raw bytes into typed payloads, and a filesystem watcher that swaps new data into existing handles in place when files change on disk. It exists so every downstream consumer — renderer, audio, scripting, editor, build — shares one identity scheme, one loading discipline, and one hot-reload mechanism rather than each inventing its own. Higher-level crates contribute their own loaders against this trait; the crate itself ships only two trivial built-in loaders and stays free of any format knowledge.
