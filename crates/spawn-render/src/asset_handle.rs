...
//! `spawn_asset::TextAsset` and texture bytes as `spawn_asset::BinaryAsset`.
//! Registering loaders for those extensions is the caller's responsibility (the
//! built-in `TextLoader` already claims `wgsl`; `BinaryLoader` claims `bin`).

use spawn_asset::{AssetId, BinaryAsset, Handle, TextAsset};
...
/// `Copy` identity of a texture source asset, derived from its [`AssetId`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
impl TextureHandle {
    ...
    /// Derives the `Copy` key from a strong texture-bytes handle. The caller
    /// retains the `Handle<BinaryAsset>` to keep the bytes alive for loading.
    pub fn from_handle(handle: &Handle<BinaryAsset>) -> Self {
        Self(handle.id())
    }
