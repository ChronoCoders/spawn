...
/// Skinned unlit forward shader: blends the four weighted joint matrices from the
/// group-2 joint storage (dynamic-offset-selected per draw) to skin the position,
/// then transforms by the per-draw model. Group 0 camera+model, group 1 material.
pub(crate) const SKINNED_OPAQUE_WGSL: &str = r#"
struct Camera { view_proj: mat4x4<f32>, view_pos: vec4<f32> };
struct Model { model: mat4x4<f32> };
struct Material { base_color: vec4<f32>, params: vec4<f32> };
...
fn skin_matrix(j: vec4<u32>, w: vec4<f32>) -> mat4x4<f32> {
    return w.x * joints[j.x] + w.y * joints[j.y] + w.z * joints[j.z] + w.w * joints[j.w];
}
...
fn vs_main(
    ...
    var out: VsOut;
    let sm = skin_matrix(joint_indices, weights);
    let skinned = sm * vec4<f32>(position, 1.0);
    out.clip = camera.view_proj * model.model * skinned;
    out.uv = uv;
    return out;
}
...
/// Skinned depth-only shadow caster: blends the four joint matrices from the
/// group-1 joint storage to skin the position, then projects by the light
/// view-projection times the per-draw model. No fragment stage.
pub(crate) const SKINNED_SHADOW_WGSL: &str = r#"
struct Camera { view_proj: mat4x4<f32>, view_pos: vec4<f32> };
struct Model { model: mat4x4<f32> };
...
fn vs_main(
    ...
    let sm = skin_matrix(joint_indices, weights);
    return camera.view_proj * model.model * sm * vec4<f32>(position, 1.0);
}
"#;

/// Skinned physically based shader: the `PBR_WGSL` Cook-Torrance fragment with a
/// skinning vertex prologue. The four joint matrices (group-3 storage,
/// dynamic-offset-selected per draw) skin position and normal before the per-draw
/// model transform. Groups 0/1/2 are camera+model / PBR material / light.
pub(crate) const SKINNED_PBR_WGSL: &str = r#"
struct Camera { view_proj: mat4x4<f32>, view_pos: vec4<f32> };
struct Model { model: mat4x4<f32> };
struct Material {
    base_color: vec4<f32>,
...
fn vs_main(
    ...
    var out: VsOut;
    let sm = skin_matrix(joint_indices, weights);
    let skinned_pos = sm * vec4<f32>(position, 1.0);
    let skinned_normal = (sm * vec4<f32>(normal, 0.0)).xyz;
